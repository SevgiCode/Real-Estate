...
if(isset($_POST['update'])){
   ...
   $update_listing = $conn->prepare("UPDATE `property` SET property_name = ?, address = ?, property_price = ?, type = ?, offer = ?, status = ?, furnished = ?, bhk = ?, deposite = ?, bedroom = ?, bathroom = ?, carpet = ?, age = ?, total_floors = ?, room_floor = ?, loan = ?, lift = ?, security_guard = ?, play_ground = ?, garden = ?, water_supply = ?, power_backup = ?, parking_area = ?, gym = ?, shopping_mall = ?, hospital = ?, school = ?, market_area = ?, description = ? WHERE id = ?");   
   $update_listing->execute([$property_name, $address, $property_price, $type, $offer, $status, $furnished, $bhk, $deposite, $bedroom, $bathroom, $carpet, $age, $total_floors, $room_floor, $loan, $lift, $security_guard, $play_ground, $garden, $water_supply, $power_backup, $parking_area, $gym, $shopping_mall, $hospital, $school, $market_area, $description, $update_id]);
   ...
   <form action="" method="POST" enctype="multipart/form-data">
      ...
      <div class="flex">
         <div class="box">
            ...
            <input type="number" name="property_price" required min="0" max="9999999999" maxlength="10" value="<?= $fetch_property['property_price']; ?>" placeholder="enter property price" class="input">
         </div>
         <div class="box">
