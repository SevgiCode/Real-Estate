...
if(isset($_POST['submit'])){
   ...
   if($select_users->rowCount() > 0){
      ...
      if($pass != $c_pass){
         ...
         $insert_user = $conn->prepare("INSERT INTO `user`(id, name, number, email, password) VALUES(?,?,?,?,?)");
         $insert_user->execute([$id, $name, $number, $email, $c_pass]);
         
         if($insert_user){
            $verify_users = $conn->prepare("SELECT * FROM `user` WHERE email = ? AND password = ? LIMIT 1");
